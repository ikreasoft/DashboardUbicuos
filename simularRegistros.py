...
"""
    ...
    """
def postJson(json):
    url = "http://localhost:3000/sessions/data"
...
"""
def simularRegistros(inicio, fin, frecuencia, extra_args, minimo=0, maximo=1):
    fecha = inicio
    data = {}
    ...
    with alive_bar(int((fin-inicio).total_seconds()/frecuencia), title='Procesando', bar='filling', spinner='wait') as bar:
        while fecha < fin:
            ...
            status, text = postJson(json.dumps(data))
            if status == 200:
                bar()
            else:
...
def comprobarArgumentos():
    ...
    return inicio, fin, frecuencia, min, max, parametros_extra

def main():
   inicio, fin, frecuencia, minimo, maximo, extra_args = comprobarArgumentos()
   simularRegistros(inicio, fin, frecuencia, extra_args, minimo, maximo)
